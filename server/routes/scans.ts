...
export const createScan: RequestHandler = async (req, res) => {
  ...
  if (!supabaseAdmin) return res.status(202).json({ saved: false, message: "Supabase not configured" });
  ...
  if (error) {
    if ((error as any)?.code === "42P01" || /relation .* does not exist/i.test(error.message)) {
      return res.status(202).json({ saved: false, message: "Table scans not found. Skipping persistence.", item: { content, meta: meta ?? null, created_at: new Date().toISOString() } });
    }
    return res.status(500).json({ error: error.message });
  }
